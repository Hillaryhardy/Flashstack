...
(define-constant POOL .flashstack-stx-pool)
...
(define-read-only (get-share-price)
  (let (
    (pool-balance (contract-call? .flashstack-stx-pool get-pool-balance))
    (total-shares (get total-shares (unwrap-panic (contract-call? .flashstack-stx-pool get-stats))))
  )
...
(define-read-only (get-total-shares)
  (get total-shares (unwrap-panic (contract-call? .flashstack-stx-pool get-stats)))
)

;; Returns total pool balance in microstacks
(define-read-only (get-pool-balance)
  (contract-call? .flashstack-stx-pool get-pool-balance)
)
...
(define-read-only (get-lp-value (lp principal))
  (let (
    (shares       (contract-call? .flashstack-stx-pool get-shares lp))
    (pool-balance (contract-call? .flashstack-stx-pool get-pool-balance))
    (total-shares (get total-shares (unwrap-panic (contract-call? .flashstack-stx-pool get-stats))))
  )
...
(define-read-only (get-lp-shares (lp principal))
  (ok (contract-call? .flashstack-stx-pool get-shares lp))
)
...
(define-read-only (get-collateral-snapshot)
  (let (
    (stats        (unwrap-panic (contract-call? .flashstack-stx-pool get-stats)))
    (pool-balance (contract-call? .flashstack-stx-pool get-pool-balance))
