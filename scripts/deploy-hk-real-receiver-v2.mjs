/**
 * FlashStack -- Deploy hk-stx-real-receiver-v2 to mainnet
 *
 * Same shape as the v1 deploy script. v2 adds a contract-caller assertion to
 * execute-stx-flash so only flashstack-stx-core can invoke the callback.
 *
 * Deploys from Hillary's own mainnet wallet (NOT the protocol deployer).
 * Admin-only whitelist call has to happen from Matt's admin wallet separately,
 * via `add-approved-receiver` on flashstack-stx-core, before v2 can be used.
 *
 * Usage:
 *   MAINNET_MNEMONIC="word1 ... word24" node scripts/deploy-hk-real-receiver-v2.mjs
 * Or, if mbegu2 file in repo root holds the 24-word mnemonic on a single line:
 *   node scripts/deploy-hk-real-receiver-v2.mjs
 *
 * Optional env:
...
const NAME     = "hk-stx-real-receiver-v2";
const PATH     = "contracts/hk-stx-real-receiver-v2.clar";
...
async function main() {
  ...
  console.log("=======================================================");
  console.log("    FlashStack -- Deploy hk-stx-real-receiver-v2       ");
  console.log("=======================================================");
  ...
  await waitForConfirm(txid, "deploy hk-stx-real-receiver-v2");
  ...
  console.log("  2. Send >=1 STX to the receiver to cover principal + fee on repay");
  console.log("  3. Call flashstack-stx-core.flash-loan(amount, receiver)");
}
