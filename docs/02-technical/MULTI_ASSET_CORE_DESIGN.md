...
**Aside — two false alarms from the same session, worth recording so they aren't mistaken for contract problems:** (1) Hillary reported PR #24's merge unexpectedly bringing in 129 files / +21,600/-4,658 lines including `pool-v3.clar` itself, `yield-vault-v5`, the Zest receiver, and `rotate-owner.mjs` — verified directly against the actual GitHub merge commit and it touched exactly the 8 originally-reviewed files, nothing else; `pool-v3.clar`'s content on `main` was confirmed byte-identical to what was already there (BC1 fix, the two §12 fixes, all present, untouched). (2) She reported running "16 tests across four files" for the sibling two-step-admin contracts; the actual count is 6 tests in one file (`tests/bc1-two-step-fix.test.ts`). Both discrepancies point at her local checkout being out of sync with upstream at review time, not at anything wrong in the repository — flagged back to her; doesn't bear on the findings below, which were verified independently regardless.
